...
class CompleteOrderAction extends CFWAction {
	...
	public function action() {
		...
		if ( ! isset( $_POST['billing_email'] ) || ! $_POST['billing_email'] ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	...
	public function add_cfw_meta_data( $order_id ) {
		...
		if ( ! empty( $_POST['billing_full_name'] ) ) {
			$order->add_meta_data( '_billing_full_name', sanitize_text_field( $_POST['billing_full_name'] ), true );
		}

		if ( ! empty( $_POST['shipping_full_name'] ) ) {
			$order->add_meta_data( '_shipping_full_name', sanitize_text_field( $_POST['shipping_full_name'] ), true );
		}
